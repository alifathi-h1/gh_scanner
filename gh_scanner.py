import time, argparse, sys, re, json, os, pyfiglet
...
class GithubScanner:
    ...
    def searchGithubRemotely(self, target):
        ...
        for pattern in patterns:
            ...
            if result_code:
                try:
                    final_result = self.analyzeResult(result_code, pattern[1], pattern[0], 'Code')
                except:
                    pass

            if result_commit:
                try:
                    final_result = self.analyzeResult(result_commit, pattern[1], pattern[0], 'Commit')
                except:
                    pass
                    ...
                    for x in ran_result:
                        while True:
                            ...
                            try:
                                ...
                                self.alerts.append({'name': ran[0], 'url': x.html_url})
                                time.sleep(1)
                                break
                            except:
                                pass
    ...
    def analyzeResult(self, result, reg, pattern_name, search_type):
        ...
        if search_type == 'Code':
            for file in result:
                ...
                for x in reg:
                    ...
                    if check:
                        print(Fore.RED)
                        print('-------------------------\n')
                        print('[--] Possible ' + pattern_name + ', Found on Code ....')
                        print("[--] URL : " + file.html_url + '\n')
                        self.alerts.append({'name': pattern_name, 'url': file.html_url})
            time.sleep(2)
            ...
            for file in result:
                print(Fore.RED)
                print('-------------------------\n')
                print('[--] Possible ' + pattern_name + ', Found on Commit ....')
                print("[--] URL : " + file.html_url + '\n')
                self.alerts.append({'name': pattern_name, 'url': file.html_url})
            time.slee(2)
    ...
    def scanMemebers(self, target):
        ...
        try:
            ...
            print('\n[++] Found ' + str(members.totalCount) + ' members, scanning for keys .....\n')
        ...
        if self.login() != False:
            ...
            if self.check_if_org_exists() == True:
                ...
                for target in self.targets:
                    ...
                    print(Fore.GREEN)
                    print('\n \n[++] The result is saved under filename: ' + str(target)+'-github-report.json')
...
print(Fore.GREEN)
banner = pyfiglet.figlet_format("GH  S c a n n e r T o o l", width=130,  justify='center')
...
parser = argparse.ArgumentParser(description='GH Scanner Tool is written in Python3 that scans Organization/User repositories for leaks such as GitHub Token, AWS Access Keys and more. \n\n')
parser.add_argument('-o', '--org', help='Organization name or user account',required=True)
parser.add_argument("-sM", "--include-members", help="scan organization's members", action="store_true")
